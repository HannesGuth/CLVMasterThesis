...
# Basically the same as "Plotting" but uses the results from the individualized PIs that come from the introduced covariate
...
for (data_list in data_lists){
    ...
    title = paste("PICP development with CET for", data_list$name)
    print(ggplot(data = cdev_data, aes(x = true_kernel)) +
            ...
            geom_line(aes(x = true_kernel + 0.03, y = CR_covered*100, color = "CR")) +
    ...
    print(ggplot(data = res_data) +
      ...
      geom_density(aes(x = CR + 0.003, color = "CR")) +
    ...
    print(ggplot(data = width_data_rel) +
      geom_line(aes(x = true_kernel, y = BS_width, color = "BS")) +
      geom_line(aes(x = true_kernel, y = EN_width, color = "EN")) +
      geom_line(aes(x = true_kernel, y = BA_width, color = "BA")) +
      geom_line(aes(x = true_kernel, y = CP_width, color = "CP")) +
      geom_line(aes(x = true_kernel + 0.03, y = CR_width, color = "CR")) +
      geom_line(aes(x = true_kernel, y = QR_width, color = "QR")) +
      scale_color_manual(values = method_colors_all) +
      labs(title = title,
           y = "PIARW",
           x = "True transactions",
