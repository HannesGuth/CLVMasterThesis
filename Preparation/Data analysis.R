for (data_list in data_lists){
  ...
  data_analysis = data.table(
    "Metric" = c("Customers", "Transactions", "Available timeframe and split in weeks", 
                 "Average number of purchases per customer", 
                 "Standard deviation of repeated purchases", 
  ...
  data_analysis[2,3] = nrow(data$data1[Date >= min(data$data1$Date) + data$l1 * 7])
  ...
  data_analysis[2,6] = nrow(data$data2[Date > min(data$data2$Date) + data$l2 * 7])
  ...
  data_analysis[4,2] = round(data_analysis[2,2] / data_analysis[1,7],2)
  data_analysis[4,3] = round(data_analysis[2,3] / data_analysis[1,7],2)
  data_analysis[4,4] = round(data_analysis[2,4] / data_analysis[1,7],2)
