...
while (ncol(LowerPI) < 40){
  tryCatch(
    {
      smp = sample(customers, 50, replace = FALSE)
      test = apparelTrans[Id %in% smp,]
      trca = apparelTrans[!(Id %in% smp),]
      ids = sample(unique(trca$Id), 120)
      ...
      quantile = quantile(conformal_data_calibrate$error, 0.9)
...
LowerPI$mean = rowMeans(LowerPI[,2:40], na.rm = TRUE)
UpperPI$mean = rowMeans(UpperPI[,2:40], na.rm = TRUE)

intervals_CP = data.table("Prediction" = results$predicted.CLV,
                          "True" = true$True,
                          "Lower" = LowerPI$mean,
                          "Upper" = UpperPI$mean)
intervals_CP = intervals_CP[complete.cases(intervals_CP),]

conformaldata_test = merge(x = conformaldata_test, y = true, by = "Id")

# Reasonable coverage for test data
sum(conformaldata_test$True > conformaldata_test$Lower & conformaldata_test$True < conformaldata_test$Upper)/nrow(conformaldata_test)

# To high coverage for all data (because models are trained with less data and because "true" data are simulated after all data)
# Based on these "true" data, errors are calculated for every smaller model
# (The smaller models might not be much worse but their predictions have probably a higher distance to the "true" values and cause larger error quantiles than the big model would need
# Therefore there is a too high coverage for all data)
sum(intervals_CP$True > intervals_CP$Lower & intervals_CP$True < intervals_CP$Upper)/nrow(intervals_CP)
