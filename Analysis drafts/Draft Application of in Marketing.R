...
for (data_list in data_lists2){
  ...
  for (method in method_list_appl){
    ...
    perf_overview[run,6] = round(mean(application_data$max_s * application_data$hul_s) / perf_overview[run,3],4)
    ...
    perf_overview[run,12] = round(mean(application_data$max_s * application_data$hll_s) / perf_overview[run,3],4)
